...
개별 계정, 서버 프로젝트/폴더, 기존 기억 가져오기와 GUI 분류, 공유/초대/철회, 인수인계, 폴더 MCP, 요금제 한도와 결제 어댑터, Flyway/배포/백업 도구를 추가했습니다. [전체 사용 흐름](service-guide.md) · [운영 설정과 미검증 항목](deployment.md) · [검증 결과](service-validation.json).

기존 기억의 자동 감시나 모든 대화의 자동 저장은 하지 않습니다. 명시적인 가져오기와 MCP 호출을 사용합니다. 공개 서버 배포·실제 결제는 환경 설정이 없어 미완료입니다. 기본 개발 실행은 demo지만 회원가입으로 개별 계정을 사용할 수 있습니다. production Compose는 service 모드로 데모 로그인을 차단합니다.
